...
export default function App() {
  ...
  const kpiKeys = [
    ...
    { key: "cac", label: "CAC", sub: "(GBP/Instruction)", fmt: fmtCurrency },
  ];
  ...
  const deltas = useMemo(() => {
    ...
    const mk = (arr: number[], invert = false) =>
      arr.map((v, i) => {
        if (i === 0) return null as number | null; // first week has no previous
        const last = arr[i - 1];
  ...
  const anomalies = useMemo(() => {
    const obj: Record<string, boolean[]> = {} as any;
    for (const k of Object.keys(deltas) as (keyof typeof deltas)[]) {
      const arr = (deltas as any)[k] as (number | null)[];
      obj[k as string] = (arr || []).map((d) => (d == null ? false : Math.abs(d) >= alertThreshold));
    }
    return obj;
  ...
  const channelRows = useMemo(() => {
    ...
    return ["paid", "organic", "email", "referral"].map((ch) => {
      const thisVal = (latestWeek.channels as any)[ch] || 0;
      const lastVal = prevWeek ? (prevWeek.channels as any)[ch] || 0 : 0;
  ...
  // ---------- Handlers ----------
  const updateWeekField = (i: number, path: string, value: any) => {
    setWeeks((cur) => {
      const next = [...cur];
      const w = { ...next[i], kpi: { ...next[i].kpi }, channels: { ...next[i].channels } } as any;
      // path: "endISO" | "kpi.revenue" | "channels.paid"
      const parts = path.split(".");
      if (parts.length === 1) {
        (w as any)[parts[0]] = value;
      } else if (parts[0] === "kpi") {
        (w.kpi as any)[parts[1]] = toNum(value);
      } else if (parts[0] === "channels") {
        (w.channels as any)[parts[1]] = toNum(value);
      }
      next[i] = w;
      return next;
    });
    ...
    <div className={`min-h-screen w-full ${present ? "bg-white" : "bg-slate-50"}`}>
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        {!present && (
          <div className="mt-4 grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <div className="p-3 bg-white rounded-2xl shadow-sm border flex items-end justify-between gap-2">
              <button onClick={copyMarkdown} className="rounded-xl border px-3 py-1.5 text-sm shadow-sm hover:bg-slate-50 active:scale-[0.99]">Copy Markdown</button>
              <button onClick={copyJSON} className="rounded-xl border px-3 py-1.5 text-sm shadow-sm hover:bg-slate-50 active:scale-[0.99]">Copy JSON</button>
            </div>
          </div>
        )}
        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {kpiKeys.map((meta) => {
            const key = meta.key as keyof typeof series;
            const latest = (latestWeek.kpi as any)[key];
            const prev = prevWeek ? (prevWeek.kpi as any)[key] : undefined;
            const delta = prevWeek ? (deltas as any)[key][(deltas as any)[key].length - 1] : null;
            const isAnom = prevWeek ? (anomalies as any)[key][(anomalies as any)[key].length - 1] : false;
            return (
              <div key={key as string} className={`p-4 rounded-2xl border bg-white shadow-sm ${isAnom ? "ring-2 ring-rose-400" : ""}`}>
                <div className="flex items-center justify-between">
                  <div>
                ...
                <div className="mt-3">
                  <Sparkline values={(series as any)[key]} height={36} />
                </div>
              </div>
            );
          })}
        ...
        {!present && (
          <div className="mt-6 bg-white border rounded-2xl shadow-sm overflow-hidden">
            <div className="px-4 py-3 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button className="rounded-xl border px-3 py-1.5 text-sm shadow-sm hover:bg-slate-50 active:scale-[0.99]" onClick={addWeek} title="Roll forward one week">+ Add Next Week (roll)</button>
              </div>
            </div>
            <div className="overflow-x-auto">
...
function Sparkline({ values, width = 220, height = 40, strokeWidth = 2 }: { values: number[]; width?: number; height?: number; strokeWidth?: number }) {
...
function NumInput({ value, onChange }: { value: number; onChange: (v: number) => void }) {
  return (
    <input
      value={value}
      onChange={(e) => onChange(toNum((e.target as HTMLInputElement).value))}
...
// ---------- Tests ----------
function runTests(weeks: any[], channelMetric: string, deltas: Record<string, (number | null)[]>) {
  ...
  const channelsOk = weeks.every((w) => {
    ...
    return vals.every(isFiniteNum) && (vals as number[]).reduce((a, b) => a + (b as number), 0) > 0;
  });
  ...
  const wowOk = Object.values(deltas).every((arr) => (arr as (number | null)[]).slice(1).every((d) => d == null || isFiniteNum(d)));
...
function sumChannels(ch: { paid?: number; organic?: number; email?: number; referral?: number }) {
...
function loadBaseline(): { docId: string; hash: string; ts: string } | null {
  try {
...
function saveBaseline(b: { docId: string; hash: string; ts: string }) {
  try {
...
function saveLastRun(ts: string) {
  try {
    localStorage.setItem("wbr_last_run", ts);
  } catch {}
}

async function canonicalHash(obj: any) {
...
function canonicalize(x: any): string {
  ...
  return `{${keys.map((k) => `${JSON.stringify(k)}:${canonicalize((x as any)[k])}`).join(",")}}`;
}
...
function buildMarkdown(
  weeks: any[],
  kpiKeys: { key: string; label: string; sub: string; fmt: (n: number) => string }[],
  ...
  kpiKeys.forEach((meta) => {
    ...
    const d = prev ? (deltas as any)[meta.key][(deltas as any)[meta.key].length - 1] : null;
